...
public class CubeCollision : MonoBehaviour
{
	...
	private void OnCollisionEnter(Collision collision)
	{
		...
		if (hasOtherCube && cube.CubeID > otherCube.CubeID)
		{
			...
			if (cube.CubeNumber == otherCube.CubeNumber)
			{
				...
				if (otherCube.CubeNumber < CubeSpawner.Instance.maxCubeNumber)
				{
					...
					var newCube = CubeSpawner.Instance.Spawn(cube.CubeNumber * 2, contactPoint + Vector3.up * 1.6f);
					audioSource.Play();
				...
				_FX.PlayCubeExplosionFX(contactPoint, cube.CubeColor);
